...
// standard:
  ...
  { "ymirror",   "blendSauce",   "vert",      "brDrift",            "pxlWidth",   "pxlHeight",   "sensitivity",   "diag",    "doubley",            "fltGray",  "macroo","",      "fltDilate",    "bdeal" },
  { "lSort",     "blendPrev",    "rstPrev",   "lameGlitch",  "echo1",      "faces",       "superlame",     "transbuffer",        "dubsiz",            "fltOpaque",     "auFader","auPhaser", "jarimg",    "slicer"}, 
  {"edge002",    "background",   "edge000",   "filter",      "interlace",  "buffer",      "rgboffset",     "poster",  "halfsiz",            "fltInvert",   "basstreble","auEcho",   "sinsin",    "stretch"},
  {"vBlur",      "rotmir",    "davai", "warp",        "mirror",     "blendbuffer", "offset",        "lameglitch2","halfx",            "fltPoster",    "wahwah","sortsort",  "sinsin2",    "stretch2"},
  {"undo",       "redo",         "vertdu",    "hordu",       "bw",         "contrast",    "brighter",      "edged",        "halfy",            "fltBlur",      "","sortsort2",  "sinsin3",    "stretch3"},
  {"horiz3","hibufcol","dus","blendbfr","selfie","sinwav","switchcolor","spielerei","terminator","VHS","lacebuf","sortsort3","brocoli",""}
};
...
color activeColor = color(200,100,0);
...
void setup() { 
  ...
  if ( frame != null ) //FIXME: catch this somehow so it doesn't get executed in jscript mode
...
void drawBuffers() { 
  if ( history.size() > 0 )
  drawBuffer(history,width-62,200,60,height-menuHeight+2,historyIdx); 
  if ( buffer.size() > 0 )
  drawBuffer(buffer,2,menuHeight+200,74,height-menuHeight-200,bufferIdx);
}
void drawBuffer(ArrayList<PImage> bfr, int x, int y, int w, int h, int idx) { //FIXME: { { i : idx, c : color }, { i : idx2, c : color } } as active toggles
  //float ratio = img1.width/img1.height;
  int s = bfr.size();
  if ( s == 0 ) return; 
  int max = 10;
  PImage testimg = aspectedResize(bfr.get(0),w,w);
  int pw = testimg.width, ph = testimg.height;
  //int pw = w, ph = (int)(w * ratio);  
  if ( s * ph > h ) max = 1+floor(h/w);
  for (int i=s-1,xp,yp,p=0; i>s-max; i--,p++) { 
        yp = y + p * ph;
        xp = x;
        try {
          if ( i < bfr.size() && i > -1 ) { 
            PImage b = bfr.get(i);
            b.loadPixels();
            PImage img = aspectedResize(b,w,w);
            img.loadPixels();
            img.copy(b,0,0,b.width,b.height,0,0,img.width,img.height);
            img.updatePixels();
            image(img,xp,yp);
        if ( i == idx-1 ) { 
            stroke(fgMenuColor);
            strokeWeight(2);
            noFill();
            rect(xp,yp,img.width,img.height);          
        }
          }
        } catch (Error e) {}
  } 
...
void setMenuHandlers() { 
  ...
  menuItems.put("sortsort",new btnSortsort());
  menuItems.put("sortsort2",new btnSortsort2());
  menuItems.put("sortsort3",new btnSortsort3());
  menuItems.put("brocoli",new btnBrocoli());
  menuItems.put("bdeal",new btnBottomDealer());
  menuItems.put("rotmir",new btnRotate());
  menuItems.put("davai",new btnDavai());
  menuItems.put("wahwah",new btnWahwah());
  menuItems.put("basstreble",new btnBassTreble());
  menuItems.put("auEcho",new btnAuEcho());
  menuItems.put("auPhaser",new btnAuPhaser());
  menuItems.put("auFader",new btnAuFader());
...
class btnAuFader implements IMenuRunnable {
  String title() { return "auFader";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = fader(img1,xPerc,yPerc);
  }
}

class btnAuPhaser implements IMenuRunnable {
  String title() { return "auPhaser";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = phaser(img1,xPerc,yPerc);
  }
}


class btnAuEcho implements IMenuRunnable {
  String title() { return "auEcho";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = auEcho(img1,xPerc,yPerc);
  }
}

class btnBassTreble implements IMenuRunnable {
  String title() { return "basstreble";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
     img1 = basstreble(img1,xPerc,yPerc);
  }
}

class btnWahwah implements IMenuRunnable {
  String title() { return "wahwah";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = wahwah(img1,xPerc,yPerc);
  }
}


class btnDavai implements IMenuRunnable {
  String title() { return "davai";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "system";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
   img1 = davai(img1,xPerc,yPerc);
  }
}


class btnRotate implements IMenuRunnable {
  String title() { return "rotmir";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "system";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
...
class btnBottomDealer implements IMenuRunnable {
  String title() { return "bdeal";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "system";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = bottomDealer(img1,xPerc,yPerc);
  }
}
...
class btnSortsort implements IMenuRunnable {
  String title() { return "sortsort";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = sortsort(img1,xPerc,yPerc);
  }
}
class btnSortsort2 implements IMenuRunnable {
  String title() { return "sortsort2";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = sortsort2(img1,xPerc,yPerc);
  }
}
class btnSortsort3 implements IMenuRunnable {
  String title() { return "sortsort3";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = sortsort3(img1,xPerc,yPerc);
  }
}

class btnBrocoli implements IMenuRunnable {
  String title() { return "brocoli";}
  Boolean  saveHistory() { return true; }
  Boolean savePreFxImage() { return true; }
  String group(){ return "glitch";}
  void run(int rowidx, int btnidx, int xPerc, int yPerc) {
    img1 = brocoli(img1,xPerc,yPerc);
  }
}
...
PImage davai(PImage img, int xp, int yp) { 
  PImage result = createImage(img.width,img.height, RGB);
  result.loadPixels(); img.loadPixels();
  for ( int i = 0, l = img.pixels.length; i<l; i++ ) { 
    color c = img.pixels[i];
    color r = color( abs(255-red(c)),  abs(255-green(c)),  abs(255-blue(c))      );
    result.pixels[i] = r;
  }
  result.updatePixels();
  return result;
...
PImage terminator(PImage img, int xp, int yp) {
  String[] terminators = { 
    "data/clipart/terminator.png",
    "data/clipart/vcr.png",
    "data/clipart/vcr2.png",
    "data/clipart/vcr3.png",
    "data/clipart/vcr4.png",
    "data/clipart/vcr5.png",
    "data/clipart/hud1.png",
    "data/clipart/hud2.png",
    "data/clipart/hud2.jpg",
    "data/clipart/hud3.jpg",
    "data/clipart/hud4.png",
    "data/clipart/game1.png",
    "data/clipart/game2.jpg",
   "data/clipart/game3.jpg",
   "data/clipart/game4.png",
   "data/clipart/game5.jpg",
   "data/clipart/test1.jpg",
   "data/clipart/robo1.png",
   "data/clipart/robo2.png",
   "data/clipart/terminator2.png" };
  //String arnold ="data/clipart/vcr2.png";// terminators[(int)(random(1)*terminators.length)];
...
PImage brocoli(PImage img, int xp, int yp) { 
  img.loadPixels();
  PImage result = createImage(img.width,img.height,RGB);
  int prevc = -200;
  int previ = img.pixels.length-1;
  int tgl = (int) map(xp,0,100,2,50);
  for ( int i = img.pixels.length -1; i>-1; i--) { 
      color c = img.pixels[i];
      if ( abs(c-prevc) >tgl ) { 
        color[] tempa = new color[previ-i+1];
        int x = 0;
        for ( int y = previ; y >= i; y--) { 
          if ( y > 0 && x > 0 ) 
          tempa[x] = img.pixels[y];
            x++;
        }
        tempa = sort(tempa);        
        x=0;
...
PImage sortsort2(PImage img, int xp, int yp) { 
  img.loadPixels();
  PImage result = createImage(img.width,img.height,RGB);
  color prevc = color(-200);
  int previ = img.pixels.length-1;
  int tgl = (int) map(xp,0,100,2,100);
  for ( int i = img.pixels.length -1; i>-1; i--) { 
      color c = img.pixels[i];
      if ( abs(brightness(c)-brightness(prevc)) >tgl ) { 
        color[] tempa = new color[previ-i+1];
        int x = 0;
        for ( int y = previ; y >= i; y--) { 
          if ( y >= 0 && x >= 0 ) 
...
PImage sortsort3(PImage img, int xp, int yp) { 
  img.loadPixels();
  PImage result = createImage(img.width,img.height,RGB);
  color prevc = color(-200);
  int previ = img.pixels.length-1;
...
PImage sortsort(PImage img, int xp, int yp) { 
  img.loadPixels();
  PImage result = createImage(img.width,img.height,RGB);
  color prevc = color(-200);
  int previ = img.pixels.length-1;
  int tgl = (int) map(xp,0,100,2,100);
  int limit = (int) map(yp,0,100,2,100);
  for ( int i = img.pixels.length -1; i>-1; i--) { 
      color c = img.pixels[i];
      if ( abs(brightness(c)-brightness(prevc)) >tgl ) { 
        color[] tempa = new color[previ-i+1];
       ...
       if ( xp > 3 )  tempa = sort(tempa);        
        x=0;
        for ( int y = previ; y >= i; y--) { 
          if ( y >= 0 && x >= 0 ) {
              result.pixels[y] = tempa[x];
            if ( y < img.pixels.length-limit ) {
              for ( int u = 0; u < limit; u++) { 
                result.pixels[y] = (int)( ( result.pixels[y] + ( img.pixels[y+u]/(u+1) )  ) / 2);
              }
              result.pixels[y] = tempa[x];
...
PImage endshuffle(PImage img, int xp, int yp) { // image, % that will be shifted, many pixels will be moved to the end/shift (max 100)
   img.loadPixels();
   int rightlimit = img.pixels.length, l = rightlimit;
   int goalpixels = (int)map(xp,0,100,0,rightlimit), shifted = 0;
   
   for ( int i = 0; i < rightlimit; i++ ) { 
     if ( shifted >= goalpixels ) break;
   }
...
PImage bottomDealer(PImage img, int xp, int yp) { // image, % that will be shifted, how many pixels will be moved to the end/shift (max 100)
   img.loadPixels();
   int rightlimit = img.pixels.length, l = rightlimit;
   int i, shifted = 0, oprtun = 0, left = rightlimit, chances = left;
   int goal =(int)map(xp,0,100,0,rightlimit);
   float chance, phi = 1.618;
   Boolean topdeal = yp>50; 
   int idx,loc;
   color oldpixel;
   if ( topdeal ) img.pixels = reverse(img.pixels);
   for ( i = 0; i < rightlimit; i++ ) { // i=0, i = 3
     if ( shifted >= goal ) break; // 0 >= 10, 0 >= 10, 
     oprtun = ( rightlimit - i ); 
     left = ( goal - shifted ); 
     chances = (oprtun - left); 
     chance = ((float)left)/(float)(exp(phi)*(chances>0?chances:1));
    if ( chances < 5 || i == (l/2) || ( chance > 0.3 && chance < 0.7 ) ||  ( i < 5 || i > rightlimit - 5 ) ) println("i: "+i+", optrun: "+oprtun +", left: "+ left +", chances: "+ chances +", shifted: "+shifted +", rightlimit: "+rightlimit +", chance: "+ chance +", goal: "+goal +". "); 
    if ( random(1.0)>chance ) {
      idx = (int)(random(1.0)*rightlimit);
      loc = rightlimit-1;
      oldpixel = img.pixels[loc];
      img.pixels[loc] = img.pixels[idx];
      img.pixels[idx] = oldpixel;
      rightlimit--;
      shifted++;
    }
    if (chances < -2 ) break;
...
PImage rotator(PImage img) { 
 PImage result = new PImage(img.height,img.width,RGB); 
   ...
   for ( int x = 0; x < img.width; x++ ) { 
     for ( int y = 0; y < img.height; y++ ) { 
       int sp = pos(x,y,img);
       int tp = pos(y,x,result);
       if ( sp > -1 && sp < img.pixels.length && tp > -1 && tp < result.pixels.length ) {
          result.pixels[tp]=img.pixels[sp];
       }
     }
   }
   result.updatePixels();
...
int pos(int x, int y, PImage img) {
  return x+y*img.width;
}



PImage aspectedResize(PImage img, int maxw, int maxh) { 
  PImage result;
  int nw,nh;
  if ( img.width >= img.height ) { //landscape or cub
    nw = maxw;
    nh = nw * img.height / img.width;
  } else { // portrait
    nh = maxh;
    nw = nh * img.width / img.height;
  }
 result = createImage(nw,nh,RGB);
...
PImage fader(PImage img, int xp, int yp) { 
  int startFade = 0, stopFade = img.pixels.length-1;
  Boolean fadeIn = false;
  // xp 0-50 = fadein, xp 50-100 = fadeout;
  // xp = start of fade (within it's 50 range)
  // yp = end of fade
  if ( xp < 51 ) { 
    fadeIn = true;
    startFade = (int) map(xp,0,50,0,img.pixels.length-1);
    stopFade = (int) map(yp,0,100,0,img.pixels.length-1);
  } else { 
    fadeIn = false;
    startFade = (int) map(xp,51,100,0,img.pixels.length-1);
    stopFade = (int) map(yp,0,100,0,img.pixels.length-1);
  }
  
  int mSample = 0, mSampleCnt = abs(stopFade - startFade);  
  PImage result = createImage(img.width,img.height,RGB);
  img.loadPixels(); result.loadPixels();
  for ( int i = 0, l = img.pixels.length; i<l; i++) { 
    color c = img.pixels[i];
    if ( i < startFade || i > stopFade ) { 
     result.pixels[i] = c;
    } else { 
      float r = map(red(c),0,255,0,1);
      float g = map(green(c),0,255,0,1);
      float b = map(blue(c),0,255,0,1);
      if ( fadeIn ) { 
         r = ( r * ((float)mSample)) / mSampleCnt;
         g = ( g * ((float)mSample)) / mSampleCnt;
         b = ( b * ((float)mSample)) / mSampleCnt;
      } else { 
         r = ( r * ((float) mSampleCnt - 1 - mSample)) / mSampleCnt;
         g = ( g * ((float) mSampleCnt - 1 - mSample)) / mSampleCnt;
         b = ( b * ((float) mSampleCnt - 1 - mSample)) / mSampleCnt;
      }
      color rc = color(
        map(r,0,1,0,255),
        map(g,0,1,0,255),
